...
— fact conditions the app evaluates from its own records, the current stage derived and never stored, the
per-area gap facts (`roleAreas`, renamed from `roleGap` 2026-09-18) untouched ([Rule 14](core-beliefs.md#rule-14)).
Four new top-level arrays: `milestones[]` (roadmap
...
`verdicts?[{ id, date, summary, basis, position, gaps[], stageK, stageN, source: "ai" }]` (newest
first, ≤ 24, AI-stated and unverified — clipped text the app never turns into a grade or payout; the raw reply
is never stored; **no `probability` field written after 2026-09-18** — third role-model change of the day, the
[Rule 14](core-beliefs.md#rule-14) amendment — though an older verdict already in the save may still carry one,
left in place unread, since no `migrate` block strips it); `seenStageK?` (a seen-stamp for the stage-completion
overlay, read by nothing but that one root effect — [Rule 9](core-beliefs.md#rule-9)). Every value the
story/verdict change derives (`roleVerdictDue`'s due state, the role verdict packet) is computed at render and
stored nowhere; `roleAreas`' filter, `role.targets` and `role.stages`' evaluator are untouched
