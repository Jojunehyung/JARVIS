...
`task.kind` ∈ book / fit / meet (`ActivityLogModal`): 독서 (reading) requires a review (star rating + ≥ 15-char impression); 미팅 (meeting) requires minutes (counterpart and agenda mandatory; action items become follow-up tasks via `spawnTask`); 운동 (fitness) records measurements optionally on first completion, one-tap afterwards. Fitness measurements (weight, skeletal muscle) are applied **only** to a same-named metric KR of an active goal through `checkinKR` — never directly to the appearance metric (Rule 8). Activity records go to the area's achievement log and pay no trophy or metric. Task templates apply kind, difficulty, and cadence together; a title/kind mismatch (`detectKind`) blocks registration; kind tasks are capped at **difficulty C** (pts < 150, so they cannot bypass the evidence gate).
