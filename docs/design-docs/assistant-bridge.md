...
| `## 업무 기록 (이월·어제·오늘)` | every undone item carried into today (`workOn(state, today, today)`) plus yesterday's items, deduped by id, date then `createdAt` ascending: `- {date} {완료\|미완료}{ · 이월 {n}일}{title}{ · 메모: note}` (the `이월` fragment only on an undone carried item; note up to `WORK_PACKET_NOTE`, 200 — the whole field; 60 until 2026-09-17; heading widened from `어제·오늘` at schema v26) | `WORK_PACKET_RECORDS` (20) |
