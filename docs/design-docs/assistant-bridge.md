...
`WORK_PACKET_HEAD` states the role, five numbered rules (facts only, `해요체`; never judge or change a score/grade/payout/difficulty; propose only today's work items, with no count limit (`건수 제한 없이`; `최대 8건` until 2026-09-17), title ≤ 60 / note ≤ 200, never create a task/event/contract/meeting, never re-propose a title already in `업무 기록`; name the goal/meeting/project a proposal is based on in `link.title`, verbatim, or omit `link`; close with a ≤ 5-line analysis then one JSON block, `"work": []` when there is nothing to propose) and the reply template
