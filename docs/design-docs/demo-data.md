...
- `act.gate` (2026-09-24, the [daily gate](../product-specs/daily-gate.md)): `{ [today − 1]: { readReaderAt: "08:31", readIssuesAt: "08:37", quiz: { total: 7, score: 5, passed: false, attempts: 1, at: "08:52" } }, [today]: { readReaderAt: "08:12", readIssuesAt: "08:19", quiz: { total: 7, score: 6, passed: true, attempts: 1, at: "08:33" }, passedAt: "08:40" } }` — today's entry carries `passedAt`, so the demo and its screenshots open in the app, not the gate; yesterday's quiz failed 5/7 (a fact stated once, never overwritten — [Rule 4](core-beliefs.md#rule-4)-adjacent immutability, though this is a stamp, not an achievement). The two entries sharing a month make `SettingsModal`'s `오늘의 관문` section read `이번 달 관문 통과 1일 · 미통과 1일 · 퀴즈 평균 5.5/7 · 미룸 0회` (the `미룸` suffix since 2026-09-26). No demo entry carries `deferredUntil`, and the demo's only appointment today is `품질 회의` at 15:00 (the others fall on other days, `○○물산 주간 점검` at 11:00 tomorrow), so the demo has no morning deferral and home shows no deferral line.
- `act.opened` (2026-09-25, the [routine guide](../product-specs/install-and-backup.md#opening-the-app-at-fixed-times--a-samsung-routine-2026-09-25)): `{ [today − 1]: "08:01", [today]: "08:04" }`, seeded directly after the `act.gate` block so the root's first-open effect finds today already stamped and leaves it — the demo therefore shows its own seeded time, not a freshly captured one. The two entries sharing a month make `SettingsModal`'s `자동 실행` section read `오늘 첫 실행 08:04 · 이번 달 실행 2일`. `settings.pushNotify` is left absent (the demo does not simulate a live push subscription).
