...
|---|---|---|---|---|
| home | 프로필 | IdCard | `HomeTab` | one CV card (identity + `프로필 편집` button + a `오늘 읽을 것 ›` button, schema v27, opening the daily reader + `설정` button, records `학력`/`경력`/`자격`/`시험`/`포트폴리오`/`성취`, area-grade rows → `PromoteModal`) · a role-model proximity line under it → the `롤모델` screen (`RoleModal`, 2026-09-18, replacing `RoleAdviceModal`) · (v28) a stage-progress headline `단계 {k}/{n} {name} · 진행 {pct}%` above the proximity line (superseded the v28 second stage line the same day, 2026-09-18), same target; no date-scoped fact of any kind (2026-09-15, [../product-specs/home.md](../product-specs/home.md)) |
