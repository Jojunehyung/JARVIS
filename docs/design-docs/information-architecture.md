...
| meetings | 미팅 | MessagesSquare | `MeetingsTab` | project-grouped, hand-written meeting minutes (schema v23; progress log and the `AI에 보내지 않기` flag since v25; structured follow-up items, mirrored to work items, since v26); header (counts + storage-use line) + `프로젝트 추가` · one section per project (`프로젝트 수정`, `회의록 추가`, compact minutes rows with a `후속 {open}/{total}` marker) · `ProjectModal` / `MeetingModal` / `MeetingViewModal`; a record, never a task — see [../product-specs/meetings.md](../product-specs/meetings.md) |
