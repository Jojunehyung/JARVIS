...
## Screens
Home (profile card + editable CV screen · today's focus = pace · today's tasks) / Goals (OKR cards · KR check-in · direction advice) / Tasks (grouped by goal: daily tasks + milestones · catalog) / Growth (skill track gates · role-model proximity · achievement wall). Onboarding in six steps (name, exact birth date, education and career records replace vague chip picks); ~10 modals; two full-screen overlays (RANK UP / ACHIEVEMENT); one toast.
