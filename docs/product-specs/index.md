...
| [daily-work.md](daily-work.md) | `업무` tab (schema v25): dated work items typed by hand or proposed by the assistant bridge and confirmed per item — day pager, `지난 미완료`, `WorkModal`, `WorkBridgeModal` (`오늘 업무 만들기`), the meeting progress log and `AI에 보내지 않기` flag that feed it, what a work item never does |
...
| [meetings.md](meetings.md) | `미팅` tab (schema v23; progress log and `AI에 보내지 않기` schema v25): project-grouped, hand-written meeting minutes — `ProjectModal`, `MeetingModal`, `MeetingViewModal`, the caps and storage-budget guard, the optional link to a `일정` event, what a meeting never does |
