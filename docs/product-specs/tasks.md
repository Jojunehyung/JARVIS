...
## `CatalogModal` — `성취 도감`
Props: `state, initialCat, initialQuery, onClose` (`initialQuery` added 2026-09-18, seeding the search input). Read-only. The `도감` button opens it on `전체`; `initialCat` is set by `RoleGradeSection`'s `도감에서 더 보기 ›` (`RoleAdviceModal`'s until 2026-09-18); `initialQuery` is set by the `롤모델` screen's `cert_held` condition landing (`도감에서 찾기 ›`), which also derives `initialCat` from `certByTitle`. Mode chips `자격증 {CERTS.length}` (1011) / `시험 {EXAMS.length}` (17).
