...
## `TaskTab`
Props: `state, today, onComplete (tryComplete), onRemove (removeTask), onCatalog, onGoGoals, onViewEvidence (opens EvidenceViewModal), onEditEvent, onToggleEventDone, onSkipEvent, onGoBiz`. `onAddFor` no longer exists — a task is never created from this tab.
...
- View chips, `할 일` / `완료` — component state (`useState`), never `state.ui` ([Rule 9](../design-docs/core-beliefs.md#rule-9)).

### `todoOf(state, today)`
The single expansion this tab and the home card ([home.md](home.md)) both read, so the two can never disagree about what "today" means. Pure — computed at render, never stored. Returns `{ groups, done, counts }`.
...
- **Task rows** come from `agendaOf(state, today).all` — the one definition of "open" the tab, the home card and the briefing all share; `date = task.due || null`.
...
The per-goal `<section>` loop, the `{goalProgress}%` figure shown in each section's header (the figure itself is unchanged and still lives in `목표` and on the home card's `오늘의 초점`, and is still read by `completeTask` / `checkinKR` / `goalStatus`), the milestone divider `마일스톤 — 자격·시험·학습 (하루분량 예외 · 증거로만 완료)`, `연결된 실행이 아직 없습니다.`, `＋ 이 목표에 실행`, and the whole `미분류 — 목표 연결 전 항목` section (with its note `완료·삭제는 가능하지만, 새 실행은 목표에서만 만들 수 있어요.`). Orphan tasks are not lost: they render as ordinary rows tagged `목표 기여 없음`, in whichever time group their `due` (or the lack of one) puts them.
