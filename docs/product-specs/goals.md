...
## Card footer
- Active: `＋ 실행 연결` → `onAddQuestFor(g.id)` (`AddTaskModal` for this goal) and, only when `pr ≥ 1`, `달성 처리` → `goalStatus(id, "done")`: the status flips, `metrics.infl += 4` and `metrics.asset += 2` (clamped 0–100 by `statClamp`), and the `achieve` overlay opens with `{name: g.title, tier: "A", kind: "rank"}`.
- Done: `기록에서 제거` → `removeGoal` filters the goal out of `goals` only. Linked tasks keep their `goalId`; because it no longer resolves they appear in the 미분류 (unassigned) section of the tasks tab. There is no delete or abandon path for an active goal.
