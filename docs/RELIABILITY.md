...
- The E2E harness asserts migration from a v10 save, a save without `v`, a v13 save, a v14 save and a v15 save to the current version (v16), including field renames, trophy-kind conversion and the empty `events[]` a v15 save gains (`tools/e2e/flow4.js`).
...
| `flow4.js` | goal completion and removal, v10 / no-version / v13 / v14 / v15 migrations |
...
| `flow7.js` | the 일정 tab: form validation, an appointment with its time, a deadline D-day, a weekly repeat (and the one-row-per-event `이후` collapse), completion mark, one cancelled occurrence, edit, delete, the briefing section, the home card line, the packet section |
...
Selectors match Korean UI copy on purpose — a copy change must update the harness in the same plan. Steps assert outcomes (completion marks, stored schema, image data URLs), not just clicks; 102 steps as of 2026-09-11.
