...
| `flow4.js` | goal completion and removal (task registered through the count-KR bridge), an active goal's deletion with its record-less tasks, v10 / no-version / v13 / v14 / v15 / v16 / v17 / v18 / v19 migrations |
...
Selectors match Korean UI copy on purpose — a copy change must update the harness in the same plan. Steps assert outcomes (completion marks, stored schema, image data URLs, calendar markers and day-number tones read from the grid, activity-kind refusal text read from the modal error), not just clicks; 135 steps as of 2026-09-13.
