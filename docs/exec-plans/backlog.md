...
2. **Real-device smoke** — now unblocked by the installable build (open the hosted address instead of the demo file); — open `release/life-demo.html` on a phone (Android: file works; iOS needs hosting) and walk: demo → task tab (per-goal groups, milestones) → goal `＋ 실행` → cert KR one-click → daily completion (delta toast) → `전기기사` milestone photo submit (S ×1.0, **+900P**) → metric check-in → reload persists. Desktop E2E covers the same path automatically (`npm run verify`). Also gains the calendar export's device check ([../design-docs/calendar-export.md](../design-docs/calendar-export.md), [install-and-backup.md](../product-specs/install-and-backup.md)): export from the installed app, open and import the `.ics` file, confirm today's digest / an all-day deadline / a timed appointment / a weekly series with its cancelled date missing, lock the phone and record whether the alarm fires at the file's time or the calendar's own default, then re-export unchanged and re-import to record whether the entry is replaced, skipped or duplicated — status **pending** until it runs.
...
4. ~~**Weekly review flow**~~ — done 2026-09-09 (`ReviewModal`, `reviews[]`, chained to the metric check-in). Original scope: — extend the metric check-in (`MetricsModal`) into a weekly screen that pairs the three sliders with a "what worked / what blocked" retrospective.
5. **Growth graphs** — due dates, journal and weekly-review records now exist; what remains is the history structure and the charts. — metric and grade history (needs a history structure in state → new migration block); onboarding final step "first goal".
...
7. ~~**PWA**~~ — done 2026-09-09: PNG and maskable icons, Apple touch icon, relative manifest scope, generated service worker (offline + update flow), persistent-storage request, and a backup file. See [../product-specs/install-and-backup.md](../product-specs/install-and-backup.md). Remaining: deploy to an HTTPS address and install on the phone.
...
- Calendar export: include an event's `place` and `note` in the `.ics` file — needs the user's own decision, since it sends more text to whatever calendar the user imports into, including a synced one ([../SECURITY.md](../SECURITY.md)).
