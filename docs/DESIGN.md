...
## Constraints
- Tailwind v3 core utilities only, no arbitrary values; icons from lucide only (in use: Trophy, Target, Plus, X, Lock, RotateCcw, Settings, Check, Star, Flag, ClipboardList, CalendarDays, Briefcase, Camera, Paperclip, Link).
...
- Invariants for any redesign: the five-tab structure (2026-09-15, down from six — the user's own decision to remove `성장`; see [decision log](design-docs/decision-log.md)) and per-screen information items; colour-role mapping; five grade colours; pace and "목표 기여 없음" always visible; evidence-first flows (no completion before attachment); the squared role-model bar whose upper segments are wider; mono numbers; dark single theme.

## Screen inventory (design handoff 2026-09-02, home rewritten 2026-09-15)
A0 style sheet · A1 onboarding title (`PROFILE SETUP`, "인생 관리", hero illustration, CTA "시작하기", demo link) · A2 onboarding steps (progress bar, chip rows, per-field errors such as "이름을 입력해 주세요." and "학력을 1개 이상 추가해 주세요." (the single combined chip-row error was retired with the age band in schema v21), appearance step with 무작위 and swatches) · A3 home (`HomeTab`: header LIFE MANAGER / nickname · status / 🔥 streak · 🛡 shields; one CV card — identity with `프로필` and `설정` buttons, records `학력`/`경력`/`자격`/`시험`/`포트폴리오`/`성취`, `영역 등급` rows that open `PromoteModal`; a role-model proximity line under the card, opening `RoleAdviceModal`) · A4 goals (`GoalsTab`: OKR cards, four KR row types, "＋ 실행 연결", achieved group; the only screen with goal progress and pace) · A5 tasks (`TaskTab`: one time-ordered list over `todoOf`, evidence-required rows, `할 일`/`완료` chips, `브리핑 열기 ›`) · A6 home settings and the achievement wall (`SettingsModal`: `롤모델` section, `데이터 — 백업 · 초기화` section; `AchievementWallModal`: counts line, trophy strip, specialisation lines, exam bests, per-area achievement lists — both opened from the home CV, replacing the retired `성장` tab) · A7 `AddTaskModal` (goal box, KR bridge rows, mode chips [일일 실행 | 학습 (하루분량)], kind chips, difficulty E/D/C, cadence, "등록") · A8 `EvidenceModal` (photo mandatory, two states) · A9 `StudyVerifyModal` (tier box, summary counter, insight, artifact box, CTA) · A10 overlays RANK UP / ACHIEVEMENT · A11 illustration sheet.
