...
## Constraints
- Tailwind v3 core utilities only, no arbitrary values; icons from lucide only (in use: Trophy, Target, Plus, X, Lock, RotateCcw, Settings, Check, Star, IdCard, ClipboardList, CalendarDays, MessagesSquare, Briefcase, Camera, Paperclip, Link). `Flag` (the old `홈` nav icon) went out 2026-09-16, replaced by `IdCard` for the renamed `프로필` tab; `MessagesSquare` came in the same day for the new `미팅` tab.
...
- Invariants for any redesign: the **six-tab structure** (2026-09-16, up from five — the user's own decision to add `미팅`; five was itself down from six on 2026-09-15, the removal of `성장`; see [decision log](design-docs/decision-log.md)) and per-screen information items; colour-role mapping; five grade colours; pace and "목표 기여 없음" always visible; evidence-first flows (no completion before attachment); the squared role-model bar whose upper segments are wider; mono numbers; dark single theme. Every nav label sits on one line at 390 px (`<span className="whitespace-nowrap leading-4">`) — six labels at `grid-cols-6` narrows each cell to about 59 px, so this is checked, not assumed.

## Screen inventory (design handoff 2026-09-02, home rewritten 2026-09-15, tasks and meetings 2026-09-16)
