...
## Constraints
- Tailwind v3 core utilities only, no arbitrary values; icons from lucide only (in use: Trophy, Target, Plus, X, Lock, RotateCcw, TrendingUp, Check, Star, Flag, ClipboardList, Camera, Paperclip, Link).
...
- Invariants for any redesign: the four-tab structure and per-screen information items; colour-role mapping; five grade colours; pace and "목표 기여 없음" always visible; evidence-first flows (no completion before attachment); the squared role-model bar whose upper segments are wider; mono numbers; dark single theme.
