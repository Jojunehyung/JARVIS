...
A0 style sheet · A1 onboarding title (`PROFILE SETUP`, "인생 관리", hero illustration, CTA "시작하기", demo link) · A2 onboarding steps (progress bar, chip rows, per-field errors such as "이름을 입력해 주세요." and "학력을 1개 이상 추가해 주세요." (the single combined chip-row error was retired with the age band in schema v21), appearance step with 무작위 and swatches) · A3 home (`HomeTab`: header LIFE MANAGER / nickname · status / 🔥 streak · 🛡 shields; profile card; "오늘의 초점" with pace; "오늘 할 일") · A4 goals (`GoalsTab`: OKR cards, four KR row types, "＋ 실행 연결", achieved group) · A5 tasks (`TaskTab`: per-goal groups, milestone divider, evidence-required rows, "＋ 이 목표에 실행", 미분류) · A6 growth (`GrowthTab`: `롤모델 근접도` headline with the squared segmented bar, skill-track rows that open `PromoteModal` on tap, collapsed `성취의 벽`, collapsed `데이터 — 백업 · 초기화`) · A7 `AddTaskModal` (goal box, KR bridge rows, mode chips [일일 실행 | 학습 (하루분량)], kind chips, difficulty E/D/C, cadence, "등록") · A8 `EvidenceModal` (photo mandatory, two states) · A9 `StudyVerifyModal` (tier box, summary counter, insight, artifact box, CTA) · A10 overlays RANK UP / ACHIEVEMENT · A11 illustration sheet.
