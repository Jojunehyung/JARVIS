...
| State lifecycle | `migrate` (v11 → v25 blocks, `@schema` JSDoc above it), `applyDailyTick`, `freshState`, `demoState` | `const migrate =` |
...
| Meetings (new 2026-09-16, schema v23; task links v24; progress log + `aiHidden` v25) | `MeetingsTab` (project sections of compact `TodoRow`-shaped minutes rows, `{n}건 더 보기` expansion, a `진행 {n}건` marker since v25), `ProjectModal`, `MeetingModal` (score field-free; caps `MEETING_LIMITS`/`PROJECT_LIMITS`, textarea counter `MeetingText`; gained the `AI에 보내지 않기` checkbox 2026-09-17), `MeetingViewModal` (gained the `진행사항` add/list/delete block and the `AI 전송` fact row 2026-09-17), helpers `meetingOrder`, `meetingEventText`, `mbText`, `taskClosedOn`, `meetingTaskCandidates`, `meetingsOfTask`, `linkedTaskLead`, constants `MEETING_ROWS_SHOWN`, `MEETING_TASK_PAST_DAYS`, `MEETING_TASK_ROWS`, `MEETING_PROGRESS_MAX` — `meetings[].taskIds` links up to 10 existing tasks, listed on the meeting view and, derived, on the task sheet's `관련 회의록`; hand-written minutes grouped by project, a record never a task; see [docs/product-specs/meetings.md](docs/product-specs/meetings.md) | `function MeetingsTab` |
| Daily work (new 2026-09-17, schema v25) | `WorkTab` (day pager, `지난 미완료` section, compact `TodoRow`s with a `수기`/`AI` lead chip), `WorkModal` (add/edit/complete/delete in one sheet), `WorkBridgeModal` (`오늘 업무 만들기` — shares `PacketSendPane`/`ReplyPastePane`/`copyPacket` with `BridgeModal`), helpers `workOn`, `workPastOpen`, `workLinkLabel`, `workLinkText`, `workLinkOptions`, `normWorkTitle`, constants `WORK_LIMITS`, `WORK_LINK_MEETINGS`, `WORK_KIND_WORD` — `work[]` is a dated record outside the goal ladder, never a task; see [docs/product-specs/daily-work.md](docs/product-specs/daily-work.md) | `function WorkTab` |
...
| App root | `LifeManager`: load → migrate → render; handlers `completeTask` (gained an optional third `score` argument 2026-09-16), `tryComplete`, `promoteArea`, `addGoal`, `goalStatus`, `checkinKR`, `setAreaDir`, `applyMeasures`, `removeTask`, `removeGoal`, `resetAll`, `saveProfile`, `addBiz`, `updateBiz`, `removeBiz`, `toggleDealPaid`, `addProject`, `updateProject`, `removeProject`, `addMeeting`, `updateMeeting`, `removeMeeting`, `putMeeting`, `recordFits` (renamed from `meetingFits` 2026-09-17, now shared by meetings, progress entries and work items, `docs/product-specs/meetings.md`), `addProgress`, `removeProgress`, `addWork`, `updateWork`, `toggleWork`, `removeWork`, `moveWorkToToday`, `importWork` (new 2026-09-17, `docs/product-specs/daily-work.md`), `downloadBlob` (the one shared create-URL/anchor/revoke download path), `exportCalendar` (builds with `buildIcs`, downloads, toasts — reads state, writes nothing); `Shell`, `NAV` (`grid-cols-7` since 2026-09-17); `setScheduleView` deleted 2026-09-16 | `export default function LifeManager` |
...
Current status (2026-09-17): schema **v25** (v22 exam scores, v23 meeting records, v24 meeting task links, v25 daily work items and meeting progress — [docs/design-docs/state-lifecycle.md](docs/design-docs/state-lifecycle.md)), difficulty table **V1.3** (`DIFF_RAW_VERSION "1.3"`, 1,011 certifications, 17 exam families, 21 jobs × 15 categories), **seven tabs** (`프로필 · 목표 · 할 일 · 업무 · 일정 · 미팅 · 사업`, `성장` retired 2026-09-15, `미팅` added 2026-09-16, `업무` added 2026-09-17 — home is one CV plus a role-model proximity line, `modal.type` 22 → 28 → 30), E2E steps **updated, not run**: `tools/e2e/flow*.js` was edited in every phase (new `flow10.js` for meetings, new `flow11.js` for daily work, renamed tabs, compact-row helpers, the exam score field, the second bridge packet) and counts 189 `await step(` calls as written (`flow.js` 38, `flow2.js` 16, `flow3.js` 14, `flow4.js` 20, `flow5.js` 16, `flow6.js` 6, `flow7.js` 25, `flow8.js` 19, `flow9.js` 8, `flow10.js` 15, `flow11.js` 12), but the standing user instruction for this change means the suite was not executed after the edits — see [docs/RELIABILITY.md](docs/RELIABILITY.md). Real-device smoke still pending (backlog 2, now including the calendar-file import and alarm check).
...
| 미팅 (프로젝트 · 회의록) | meeting minutes — project (`meetingProjects`) and its hand-written minutes (`meetings`, `MeetingsTab`, `ProjectModal`/`MeetingModal`/`MeetingViewModal`, schema v23; `taskIds` links existing tasks, schema v24; `progress[]`/`aiHidden` schema v25); a record, never a task, no time of its own — see 일정 above |
| 업무 | daily work item — a dated record outside the goal ladder (`work`, `WorkTab`, `WorkModal`, `WorkBridgeModal`, schema v25); typed by hand or proposed by the assistant bridge and confirmed per item; a record, never a task |
