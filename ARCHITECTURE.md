...
| State lifecycle | `migrate` (v11 → v19 blocks, `@schema` JSDoc above it), `applyDailyTick`, `freshState`, `demoState` | `const migrate =` |
...
| App root | `LifeManager`: load → migrate → render; handlers `completeTask`, `tryComplete`, `promoteArea`, `addGoal`, `goalStatus`, `checkinKR`, `setAreaDir`, `applyMeasures`, `removeTask`, `removeGoal`, `resetAll`; `Shell`, `NAV` | `export default function LifeManager` |
...
Current status (2026-09-11): schema **v19**, difficulty table **V1.3** (`DIFF_RAW_VERSION "1.3"`, 1,011 certifications, 17 exam families, 21 jobs × 15 categories), E2E 116 steps green, real-device smoke still pending (backlog 2).
